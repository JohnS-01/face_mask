...
def make_pw_hash(pw, salt=None):
    """ Creates the actual password hash with the salt included.
        Takes the entered password as a string, and an optional salt, also a string, which defaults to None. """
    if not salt:
        salt = make_salt()
    new_hash = hashlib.sha256(str.encode(pw + salt)).hexdigest()  # creates a unique user hash from the combined pw and salt
    return '{0},{1}'.format(new_hash, salt)  # returns the new hash, including the salt so user can be verified later

def check_pw_hash(pw, hash):
    ...
    salt = hash.split(',')[1]  # gets the users original salt so that when it creates the new hash it will match
    if make_pw_hash(pw, salt) == hash:
        return True
    return False